...
parsePassTokens()
{
  ...
  if (g.opt.password != NULL) {
    char* temp;
    int len = strlen(g.opt.password);
    char *temp2 = (char*)malloc(sizeof(char) * (len + 1));
    memcpy(temp2, g.opt.password, len);
    char* temp3 = temp2;
    while((temp = strtok(temp3, " \r\n")) != NULL) {
	temp3 = NULL;
	struct Token *token = new (struct Token, NULL, temp); 
	if (g.pos == NULL) {
	   g.tokens = token;
	   g.pos = token;
	} else {
...
L_chk_sigchld:
        ...
        if (FD_ISSET(g.fd_ptym, &readfds) ) {
			...
			if (g.pos != NULL) {
			    char* cur = (g.pos)->pass;
			    g.pos = (g.pos)->next;
			    write(g.fd_ptym, cur, strlen(cur));
			}
                        write(g.fd_ptym, "\r", 1);
